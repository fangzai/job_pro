...
public class BinaryLinkTree {
    public TreeLinkNode root = null;
    public TreeLinkNode createBinaryTree(int[] nums) {
    ...
    public TreeLinkNode createBinaryTree(int[] nums, int len) {
        if (nums.length < len) {
            return null;
        }
        //用先序遍历的方式创建二叉树，遇到-1则表示该节点为null
    ...
    private TreeLinkNode helpTree(int[] index, int[] data, int len) {
        /*
        ...
        * */
        ...
        if (data[index[0]] != -1) {
            current = new TreeLinkNode(data[index[0]]);
            index[0]++;
            if(index[0] < len) {
